...
export const getDataGajiPegawai = async () => {
  try {
    ...
    const potongan_pegawai = resultDataKehadiran.map((kehadiran) => {
      const potonganUnexcused = kehadiran.alpha > 0 ?
        resultDataPotongan
          .filter((potongan) => potongan.nama_potongan.toLowerCase() === "unexcused")
          .reduce((total, potongan) => total + potongan.jml_potongan * kehadiran.alpha, 0) : 0;

      const potonganSick = kehadiran.sakit > 0 ?
        resultDataPotongan
          .filter((potongan) => potongan.nama_potongan.toLowerCase() === "sick")
      ...
      return {
        ...
        potonganSick: potonganSick,
        potonganUnexcused: potonganUnexcused,
        total_potongan: potonganSick + potonganUnexcused
      };
    });
