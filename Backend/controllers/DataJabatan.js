...
export const getDataJabatan = async (req, res) => {
    try {
        ...
        if (req.hak_akses === "admin") {
            ...
            if (req.userId !== DataJabatan.userId) return res.status(403).json({ msg: "Forbidden access" });
...
export const createDataJabatan = async (req, res) => {
    ...
    try {
        if (gaji_pokok <= 0 || tj_transport <= 0 || uang_makan <= 0) {
            return res.status(400).json({ success: false, message: "Salary and allowance values must be positive numbers" });
        }
        if (req.hak_akses === "admin") {
        ...
        res.status(201).json({ success: true, message: "Position data successfully saved" });
    } catch (error) {
...
export const updateDataJabatan = async (req, res) => {
    try {
        ...
        if (gaji_pokok <= 0 || tj_transport <= 0 || uang_makan <= 0) {
            return res.status(400).json({ msg: "Salary and allowance values must be positive numbers" });
        }
        ...
        res.status(200).json({ msg: "Position data successfully updated" });
    } catch (error) {
        res.status(500).json({ msg: error.message });
    }
}
